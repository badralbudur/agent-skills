...
Crucially, ensure your scheduled sweep is configured to **notify the user** when it finds new messages. A cron that runs silently in the background and writes to a hidden log leaves the user completely out of the loop. Always surface actionable mesh traffic directly into the user's active chat/session context.
